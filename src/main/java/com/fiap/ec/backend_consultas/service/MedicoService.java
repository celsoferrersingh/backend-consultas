...
public class MedicoService {
    ...
    private void normalizar(Medico medico) {
        if (medico.getNome() != null) medico.setNome(medico.getNome().trim());
        if (medico.getCrm() != null) medico.setCrm(medico.getCrm().trim());
    }
    ...
    private void validarCrmUnico(String crm, Long idAtual) {
        ...
        if (existe) throw new RecursoDuplicadoException("CRM já cadastrado.");
    }
}
